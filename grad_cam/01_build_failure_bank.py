"""
Task 2: 실패 사례 은행(Failure Bank) — grad_cam 전용

상위 스크립트와 동일 로직. CSV·annotated 는 grad_cam/failure_bank/ 에 저장.
선별 이미지는 상위 test_images/ 로 복사(이미 있으면 생략).
...
def main() -> None:
    ...
    print(f"[INFO] {len(images)}장 → 상위 test_images/ (복사 또는 기존), 분류 결과 → grad_cam/failure_bank/")
